...
LOGS_DIR="${PROJECT_DIR}/logs/proc_dwi"
if [[ "$dataset" == "pnc" ]]; then
    OUTPUT_DIR="${PNC_PROJECT_DIR}/output"
fi
# Note: for imagen OUTPUT_DIR will be defined in the for loop depending on $ses
...
for sub_orig in $all_subjects; do
    ...
    if [ ! "$(ls -A ${SC_DIR}/${sub} 2>/dev/null)" ]; then 
        ...
        if [ $(echo $current_condor_jobs | grep "${PROJECT_DIR}/scripts/proc_dwi/proc_dwi.sh ${dataset} ${sub_orig}" | wc -l) -gt 0  ]; then
            continue
        fi
        printf "arguments = ${PROJECT_DIR}/scripts/proc_dwi/proc_dwi.sh ${dataset} ${sub_orig}\n"
